
<h1 align="center"> PiggyPocket</h1>
<h3 align="center"> Budgeting App</h3>

<p align="center">
  OPSC6311 POE <br>
  Budget well. Live life. Do good.
</p>


##  About the App  

PiggyPocket is a simple and user-friendly budgeting app designed to help users manage their finances effectively.  
It allows users to track income, monitor expenses, and maintain financial discipline.

---

## Project Links  

<p align="center">
  <a href="https://github.com/ST10448358/PiggyPocket_Part3.git">
    <img src="https://img.shields.io/badge/View%20Code-GitHub-black?style=for-the-badge&logo=github"><br>
  </a>

  <a href="https://youtu.be/9hrFypdLx2o">
    <img src="https://img.shields.io/badge/Watch%20Demo-YouTube-red?style=for-the-badge&logo=youtube">
  </a>
</p>


Click here if icon is not working: Github link:https://github.com/ST10448358/PiggyPocket_Part2.git

Click here if icon is not working: Youtube video link:https://youtu.be/9hrFypdLx2o




## Installation & Setup  

Follow these steps to run the project using Android Studio:

1. Install Android Studio  
2. Open Android Studio  
3. Click **Open Project**  
4. Select the project folder  
5. Allow Gradle to sync  
6. Click ▶️ Run to start the app  



## Authors
Group 4: 
- Katlego Molokwane
- Lesedi Mathunya



## Additional Documentation

- [Notifications Feature](docs/Notifications-Feature-README.md)
- [Wishlist Planner Feature](docs/Wishlist-Planner-README.md)
